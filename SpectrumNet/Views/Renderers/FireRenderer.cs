...
public sealed class FireRenderer : BaseSpectrumRenderer
{
    ...
    private float[] ScaleSpectrum(float[] spectrum, int targetCount, int spectrumLength)
    {
        ...
        {
            int chunkSize = Min(Constants.SPECTRUM_PROCESSING_CHUNK_SIZE, targetCount);
    ...
    private class RenderScope : IDisposable
    {
            ...
            SKPaint basePaint)
        {
            ...
            _glowPaint.MaskFilter = SKMaskFilter.CreateBlur(Normal, _glowRadius);
        ...
        private float CalculateTotalBarWidth(int actualBarCount)
        {
            ...
            return actualBarCount < Constants.MIN_BAR_COUNT
                ? totalBarWidth * Constants.MIN_BAR_COUNT / actualBarCount
                : totalBarWidth;
        }
        ...
        private FlameParameters CalculateFlameParameters(int index, float totalBarWidth, float spectrumValue)
        {
            ...
            float waveOffset = (float)Sin(_renderer._time * Constants.WAVE_SPEED + index * Constants.POSITION_PHASE_SHIFT)
        ...
        {
            float flameTop = parameters.CanvasHeight - Max(parameters.CurrentHeight, parameters.PreviousHeight);
        ...
        {
            float waveOffset = (float)Sin(_renderer._time * Constants.WAVE_SPEED +
        ...
        {
            float opacityWave = (float)Sin(_renderer._time * Constants.OPACITY_WAVE_SPEED +
