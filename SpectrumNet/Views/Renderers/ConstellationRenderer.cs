...
public sealed class ConstellationRenderer : BaseSpectrumRenderer
{
    ...
    private SKPaint CreateGlowPaint(SKColor color, float radius, byte alpha)
    {
        return new SKPaint
        {
            ...
            IsAntialias = _useAntiAlias,
            Style = Fill,
            MaskFilter = SKMaskFilter.CreateBlur(Normal, radius)
        };
    }
        ...
        Safe(() =>
        {
            ...
            lock (_starsLock)
            {
                for (int i = 0; i < _stars.Length; i++)
                {
                    ...
                    float newOpacity = Max(star.Opacity - Constants.TIME_STEP * Constants.FADE_OUT_SPEED, 0);
                    ...
                    else
                    {
                        ...
                        float newLifetime = Max(star.Lifetime - Constants.TIME_STEP * 2, 0);
        ...
        Safe(() =>
        {
            ...
            lock (_starsLock)
            {
                for (int i = 0; i < _stars.Length; i++)
                {
                    ...
                    // Fade in new stars
                    float newOpacity = Min(star.Opacity + Constants.TIME_STEP * Constants.FADE_IN_SPEED, 1.0f);
    ...
    private void CalculateEdgeRepulsion(in Star star, float screenWidth, float screenHeight, ref float forceX, ref float forceY)
    {
        ...
        float edgeDistX = Min(star.X, screenWidth - star.X);
        float edgeDistY = Min(star.Y, screenHeight - star.Y);
    ...
    private void ProcessSpectrum(float[] spectrum)
    {
        Safe(() =>
        {
            ...
            _processedSpectrum = Min(_midSpectrum * Constants.SPECTRUM_AMPLIFICATION, Constants.MAX_BRIGHTNESS);
