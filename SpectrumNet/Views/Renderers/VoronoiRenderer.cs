...
public sealed class VoronoiRenderer : BaseSpectrumRenderer
{
        ...
        Action<SKCanvas, SKImageInfo>? drawPerformanceInfo)
    {
        ...
        Safe(() =>
        {
            ...
            try
            {
                ...
                {
                    int freqBands = Min(spectrum!.Length, Constants.DEFAULT_POINT_COUNT);
                    ...
                    {
                        _gridCols = (int)Ceiling((float)info.Width / Constants.GRID_CELL_SIZE);
                        _gridRows = (int)Ceiling((float)info.Height / Constants.GRID_CELL_SIZE);
                ...
                for (int row = 0; row < _gridRows; row++)
                {
                    for (int col = 0; col < _gridCols; col++)
                    {
                        ...
                        if (nearestIndex >= 0 && nearestIndex < _voronoiPoints!.Count)
                        {
                            ...
                            byte a = (byte)Clamp(Constants.MIN_CELL_ALPHA + (int)(point.Size * Constants.ALPHA_MULTIPLIER), 0, 255);
                            ...
                            canvas.DrawRect(
                                cellX,
                                cellY,
                                Min(Constants.GRID_CELL_SIZE, info.Width - cellX),
                                Min(Constants.GRID_CELL_SIZE, info.Height - cellY),
                                _cellPaint
                            );
                        }
                    }
    ...
    private void DrawVoronoiBorders(SKCanvas canvas, SKImageInfo info)
    {
        Safe(() =>
        {
            float maxDistance = Max(info.Width, info.Height) * Constants.MAX_DISTANCE_FACTOR;
    ...
    private void ProcessSpectrum(float[] spectrum, int freqBands)
    {
        Safe(() =>
        {
            ...
            for (int i = 0; i < freqBands; i++)
            {
                ...
                endBin = Min(endBin, spectrum.Length);

                float sum = 0;

                // Use SIMD for large chunks if available
                if (IsHardwareAccelerated && endBin - startBin >= Vector<float>.Count)
                {
                ...
                _processedSpectrum[i] = Clamp(_processedSpectrum[i], 0, 1);
            }
    ...
    private void UpdateVoronoiPoints(float width, float height)
    {
        Safe(() =>
        {
            ...
            for (int i = 0; i < _voronoiPoints.Count; i++)
            {
                ...
                point.X += point.VelocityX * (float)Sin(_timeAccumulator + i);
                point.Y += point.VelocityY * (float)Cos(_timeAccumulator * 0.7f + i);
