...
using SpectrumNet.SN.Visualization.Interfaces;
using static SpectrumNet.SN.Visualization.Utils.RendererPlaceholder.Constants;

namespace SpectrumNet.SN.Visualization.Utils;
...
public sealed class RendererPlaceholder : IPlaceholder
{
        ...
        (float h, float s, float l) hsl2, float t)
    {
        ...
        return (h, Clamp(s, 0, 100), Clamp(l, 0, 100));
    }
    ...
    private static byte GetNextColorComponent(byte baseColorComponent) =>
        (byte)Clamp((float)baseColorComponent + Random.Shared.Next(-COLOR_RANDOM_DELTA_RANGE,
            COLOR_RANDOM_DELTA_RANGE + 1), COLOR_CLAMP_MIN, COLOR_CLAMP_MAX);
