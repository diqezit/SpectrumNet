...
public sealed class Renderer : IDisposable
{
    ...
    private SpectralData? GetSpectrumData()
    {
        ...
        var result = Safe(() => analyzer.GetCurrentSpectrum(),
    ...
    private bool TryCalcRenderParams(SKImageInfo info, out float barWidth, out float barSpacing, out int barCount)
    {
        ...
        barSpacing = Min((float)_controller.BarSpacing, totalWidth / (barCount + 1));
        barWidth = Max((totalWidth - (barCount - 1) * barSpacing) / barCount, 1.0f);
