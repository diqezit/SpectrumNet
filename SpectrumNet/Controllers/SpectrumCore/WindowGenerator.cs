...
{
    private const string LOG_PREFIX = nameof(WindowGenerator);
        ...
        type switch
        {
            ...
            FftWindowType.Blackman =>
                i => window[i] = 0.42f - 0.5f * cosTable[i] +
                    0.08f * MathF.Cos(Constants.TWO_PI * 2 * i / (size - 1)),
            ...
            FftWindowType.Kaiser =>
                i => window[i] = BesselI0(Constants.KAISER_BETA *
                    MathF.Sqrt(1 - MathF.Pow(2f * i / (size - 1) - 1, 2))) /
                    BesselI0(Constants.KAISER_BETA),
    ...
    private static float BesselI0(float x)
    {
        ...
        for (int k = 1; term > Constants.BESSEL_EPSILON; k++)
        {
            sum += term;
            term *= x * x / (4f * k * k);
        }
