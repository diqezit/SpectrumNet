    ...
    public sealed class CubeRenderer : BaseSpectrumRenderer
    {
        ...
        private void UpdateCubeSize()
        {
            SmartLogger.Safe(() =>
            {
                ...
                targetSize = Math.Clamp(targetSize, Constants.MIN_CUBE_SIZE, Constants.MAX_CUBE_SIZE);
        ...
        private void UpdateRotationSpeeds()
        {
            SmartLogger.Safe(() =>
            {
                ...
                _rotationSpeedX = Math.Min(_rotationSpeedX, Constants.MAX_ROTATION_SPEED);
                _rotationSpeedY = Math.Min(_rotationSpeedY, Constants.MAX_ROTATION_SPEED);
                _rotationSpeedZ = Math.Min(_rotationSpeedZ, Constants.MAX_ROTATION_SPEED);
            }, new SmartLogger.ErrorHandlingOptions
            {
        ...
        private void CalculateFaceDepthsAndNormals()
        {
            SmartLogger.Safe(() =>
            {
                for (int i = 0; i < _faces.Length; i++)
                {
                    ...
                    lightIntensity = Constants.AMBIENT_LIGHT + Constants.DIFFUSE_LIGHT * Math.Max(0, lightIntensity);
