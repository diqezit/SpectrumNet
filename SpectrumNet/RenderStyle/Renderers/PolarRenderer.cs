...
namespace SpectrumNet
{
    ...
    public sealed class PolarRenderer : BaseSpectrumRenderer
    {
        ...
        private static class Constants
        {
            ...
            public const float DEG_TO_RAD = (float)(Math.PI / 180.0); // Degrees to radians conversion
            ...
            SmartLogger.Safe(() =>
            {
                ...
                if (overlayChanged)
                {
                    _currentPointCount = isOverlayActive
                        ...
                        : Math.Min(_maxPoints, Constants.MAX_POINT_COUNT);
                    _pathsNeedUpdate = true;
                }
        ...
        protected override void ApplyQualitySettings()
        {
            ...
            switch (_quality)
            {
                ...
                SmartLogger.Log(LogLevel.Debug, Constants.LOG_PREFIX, $"Quality changed to {_quality}");
            }, new SmartLogger.ErrorHandlingOptions
            {
            ...
            SmartLogger.Safe(() =>
            {
                ...
                if (_useHighlight)
                {
                    _highlightPaint.Color = new SKColor(
                        (byte)Math.Min(255, baseColor.Red * Constants.HIGHLIGHT_FACTOR),
                        (byte)Math.Min(255, baseColor.Green * Constants.HIGHLIGHT_FACTOR),
                        (byte)Math.Min(255, baseColor.Blue * Constants.HIGHLIGHT_FACTOR),
            ...
            Action<SKCanvas, SKImageInfo>? drawPerformanceInfo)
        {
            ...
            SmartLogger.Safe(() =>
            {
                int safeBarCount = Math.Min(Math.Max(barCount, Constants.MIN_POINT_COUNT), _maxPoints);
                float safeBarWidth = Math.Clamp(barWidth, Constants.MIN_BAR_WIDTH, Constants.MAX_BAR_WIDTH);
        ...
        private void RenderPolarGraph(SKCanvas canvas, SKImageInfo info, SKPaint basePaint, float barWidth)
        {
            ...
            if (_outerPath == null || _innerPath == null ||
                ...
                if (_usePulseEffect)
                {
                    _pulseEffect = (float)Math.Sin(_time * Constants.PULSE_SPEED) *
                ...
                if (!canvas.QuickReject(_clipBounds))
                {
                    ...
                    float pulseScale = _usePulseEffect
                        ? 1.0f + (float)Math.Sin(_time * Constants.PULSE_SPEED * 0.5f) * 0.1f
                        : 1.0f;
        ...
        private void UpdatePolarPaths(SKImageInfo info, int barCount)
        {
            SmartLogger.Safe(() =>
            {
                ...
                for (int i = 0, pointIndex = 0; i <= effectivePointCount; i += skipFactor, pointIndex++)
                {
                    ...
                    else
                    {
                        modulation = 1 + Constants.MODULATION_FACTOR *
                            (float)Math.Sin(pointIndex * angleStep * Constants.MODULATION_FREQ * Constants.DEG_TO_RAD + _time * 2);

                        if (_usePulseEffect)
                        {
                            modulation += Constants.PULSE_AMPLITUDE * 0.5f * (float)Math.Sin(timeOffset);
                        }
                    }
                    ...
                    else
                    {
                        innerModulation = 1 + Constants.MODULATION_FACTOR *
                            (float)Math.Sin(pointIndex * angleStep * Constants.MODULATION_FREQ * Constants.DEG_TO_RAD + _time * 2 + Math.PI);

                        if (_usePulseEffect)
                        {
                            innerModulation += Constants.PULSE_AMPLITUDE * 0.5f * (float)Math.Sin(timeOffset + Math.PI);
                        }
                    }
                ...
                {
                    SmartLogger.Log(LogLevel.Error, Constants.LOG_PREFIX, $"Failed to create path: {ex.Message}");
        ...
        private void ExtractSpectrumPoints(float[] spectrum, int pointCount)
        {
            SmartLogger.Safe(() =>
            {
                ...
                {
                    SmartLogger.Log(LogLevel.Error, Constants.LOG_PREFIX, "Temporary spectrum array is null");
                    return;
                }
                ...
                for (int i = 0; i < pointCount && i < _tempSpectrum.Length; i++)
                {
                    ...
                    _tempSpectrum[i] = Math.Min(_tempSpectrum[i] * Constants.SPECTRUM_SCALE, Constants.MAX_SPECTRUM_VALUE);
                }
            }, new SmartLogger.ErrorHandlingOptions
            {
        ...
        private float SmoothSpectrumSIMD(int pointCount)
        {
            ...
            {
                SmartLogger.Log(LogLevel.Error, Constants.LOG_PREFIX, "Spectrum data arrays are null");
                return;
            }
                ...
                for (int i = 0; i < safePointCount; i += Vector<float>.Count)
                {
                    ...
                    if (remaining < Vector<float>.Count)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                SmartLogger.Log(LogLevel.Error, Constants.LOG_PREFIX, "Null reference in SIMD processing");
