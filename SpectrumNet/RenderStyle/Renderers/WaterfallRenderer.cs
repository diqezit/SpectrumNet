...
namespace SpectrumNet
{
    ...
    public sealed class WaterfallRenderer : BaseSpectrumRenderer
    {
                    ...
                    if (Vector.IsHardwareAccelerated && spectrumWidth >= Vector<float>.Count)
                    {
                        Parallel.For(0, bufferHeight, y =>
                        {
                            ...
                            for (int x = 0; x < vectorizedWidth; x += Vector<float>.Count)
                            {
                                ...
                                if (Math.Abs(colorEnhancementFactor - 1.0f) > float.Epsilon)
                                    values = Vector.Multiply(values, colorEnhancementFactor);
        ...
        private float[] AdjustSpectrumResolution(float[] spectrum, int barCount, float barWidth)
        {
            ...
            float detailLevel = Math.Max(0.1f, Math.Min(1.0f, barWidth * Constants.DETAIL_SCALE_FACTOR));
        ...
        {
            int baseWidth = Math.Max(barCount, Constants.MIN_BAR_COUNT);
            ...
            return Math.Max(width, Constants.DEFAULT_SPECTRUM_WIDTH / 4);
        }
