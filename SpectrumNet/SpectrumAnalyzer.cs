...
public sealed class SpectrumAnalyzer : ISpectralDataProvider, ISpectrumAnalyzer, IDisposable, IComponent
{
    ...
    public SpectrumAnalyzer(
        ...
        SynchronizationContext? context = null)
    {
        ArgumentNullException.ThrowIfNull(fftProcessor, nameof(fftProcessor));
    ...
    private void OnFftCalculated(object? sender, FftEventArgs e)
    {
        ...
        {
            SmartLogger.Log(LogLevel.Error, LogPrefix, $"Error converting FFT to spectrum: {ex}");
            return;
        }
        ...
        try
        {
            ...
            if (context != null)
            {
                context.Post(_ =>
                {
                    ...
                    {
                        SmartLogger.Log(LogLevel.Error, LogPrefix, $"InvokeEvent: error in handler: {ex.Message}");
                    }
                }, null);
            }
            else
...
public sealed class FftProcessor : IFftProcessor, IAsyncDisposable
{
    ...
    public FftProcessor(int fftSize = Constants.DefaultFftSize)
    {
        ...
        foreach (FftWindowType type in Enum.GetValues<FftWindowType>())
        {
    ...
    private async Task ProcessAsync()
    {
        ...
        catch (OperationCanceledException) { /* Cancellation is expected during disposal */ }
        catch (Exception ex)
        {
    ...
    private void ProcessBatch(ReadOnlyMemory<float> samples, int rate)
    {
        ...
        while (pos < samples.Length)
        {
            ...
            if (_sampleCount >= _fftSize)
            {
                ...
                {
                    SmartLogger.Log(LogLevel.Error, LogPrefix, $"FFT calculation failed: {ex}");
                    ResetFftState();
                    return;
                }
...
public sealed class SpectrumConverter : ISpectrumConverter
{
    ...
    {
        ArgumentNullException.ThrowIfNull(fft, nameof(fft));
    ...
    #region Frequency Conversion Methods
