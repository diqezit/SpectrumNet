...
namespace SpectrumNet
{
    ...
    public readonly struct SpectrumParameters
    {
        ...
        public static SpectrumParameters FromProvider(IGainParametersProvider provider) =>
            new SpectrumParameters(provider.MinDbValue, provider.MaxDbValue, provider.AmplificationFactor);
    }
    ...
    public sealed class GainParameters : IGainParametersProvider, INotifyPropertyChanged
    {
        ...
        {
            get => _amp;
            set => UpdateProperty(ref _amp, MathF.Max(0.1f, value));
        }

        public float MaxDbValue
        {
            get => _max;
            set => UpdateProperty(ref _max, MathF.Max(value, _min));
        }

        public float MinDbValue
        {
            get => _min;
            set => UpdateProperty(ref _min, MathF.Min(value, _max));
        }
    ...
    public sealed class SpectrumAnalyzer : ISpectralDataProvider, IDisposable, IComponent
    {
        ...
        public async Task AddSamplesAsync(float[] samples, int sampleRate, CancellationToken cancellationToken = default)
        {
            ...
            {
                Log.Error($"{LogPrefix}Error adding samples: {ex}");
                throw;
            }
        }
        ...
        private void OnFftCalculated(object? sender, FftEventArgs e)
        {
            ...
            {
                Log.Error($"{LogPrefix}FFT callback error: {ex}");
            }
        }
        ...
        public void Dispose()
        {
            if (!_disposed)
            {
                ...
                {
                    Log.Error($"{LogPrefix}Dispose error: {ex}");
                }
    ...
    public sealed class FftProcessor : IFftProcessor, IAsyncDisposable
    {
        ...
        private (float[] cos, float[] sin) PrecomputeTrigonometricValues(int size)
        {
            ...
            float[] cos = _pool.Rent(size);
            float[] sin = _pool.Rent(size);
        ...
        private async Task ProcessAsync()
        {
            try
            {
                await foreach (var (samples, rate) in _channel.Reader.ReadAllAsync(_cts.Token))
                {
                    if (samples.Length > 0)
                    {
                        ...
                        {
                            Log.Error($"{LogPrefix}Batch error: {ex}");
                        }
                    }
                }
            }
            ...
            {
                // Операция отменена – никаких действий не требуется
            }
            catch (Exception ex)
            {
                Log.Error($"{LogPrefix}ProcessAsync error: {ex}");
            }
        }
        ...
        private void ProcessChunk(ReadOnlySpan<float> chunk)
        {
            ...
            for (int i = 0; i < vecCount; i++)
            {
                ...
                Vector<float> sampleVec = new Vector<float>(temp);
                Vector<float> windowVec = new Vector<float>(_window.AsSpan(_sampleCount + offset, _vecSize));

                Vector<float> resultVec = sampleVec * windowVec;
            ...
            {
                _buffer[_sampleCount + start + i] = new Complex
                {
                    X = chunk[start + i] * _window[_sampleCount + start + i]
                };
            }
        }
        ...
        private void PerformFftCalculation(int rate)
        {
            ...
            {
                Log.Error($"{LogPrefix}FFT error: {ex}");
            }
        }
    }
    ...
    public sealed class SpectrumConverter : ISpectrumConverter
    {
        ...
        private static readonly Vector<float> _tenVec = new(10f);
        private static readonly Vector<float> _invLog10Vec = new(Constants.InvLog10);
        private static readonly Vector<float> _epsilonVec = new(float.Epsilon);
        ...
        private void ProcessSpectrum(Complex[] fft, float[] spectrum, SpectrumRange range)
        {
            ...
            Parallel.ForEach(
                Partitioner.Create(range.MinIndex, vectorEnd, _vectorSize),
                _parallelOpts,
                part =>
                {
        ...
        private static void ProcessSpectrumVector(Complex[] fft, float[] spectrum, int index, int minIdx, SpectrumParameters sp)
        {
            ...
            Vector<float> logMagVec = new(temp);
            ...
            Vector<float> adjusted = new(temp);
